...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Markoni AI Stay — Private room & terrace near Bangalore Airport" },
      {
        property: "og:description",
        content: "A calm private stay minutes from Bangalore Airport. Self check-in, terrace, fast Wi-Fi.",
      },
...
function Home() {
  ...
  const { rooms, highlights, gallery, reviews, faqs, nearby, info, amenities } = data;
  const lowest = rooms.length ? Math.min(...rooms.map((room) => room.price_per_night)) : 0;
  ...
  return (
    <>
      ...
      <section className="relative flex min-h-[92vh] items-end overflow-hidden">
        <img
          src={gallery[1]?.url ?? gallery[0]?.url ?? ""}
          alt={gallery[1]?.alt ?? "The terrace at Markoni AI Stay"}
        ...
        <div className="shell relative w-full pb-20 pt-36 text-ink-foreground">
          <p className="eyebrow text-ink-foreground/70">Tharabanahalli · Bangalore North</p>
          ...
          <p className="mt-7 max-w-xl text-base text-ink-foreground/80">
            A room of your own, a terrace under an open sky, and self check-in at any hour — a short drive
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            ...
            <span className="text-sm text-ink-foreground/70">From {formatINR(lowest)} a night</span>
          </div>
        </div>
      </section>
      ...
      <section className="shell grid gap-14 py-28 lg:grid-cols-12 lg:py-36">
        <Reveal className="lg:col-span-5">
          ...
          <h2 className="mt-5 font-display text-4xl sm:text-5xl">
            A quiet house on the airport road, kept the way we'd want to arrive.
          </h2>
        </Reveal>
        <Reveal className="space-y-6 text-muted-foreground lg:col-span-6 lg:col-start-7" delay={120}>
          <p>
            Zoe is a family home set behind a planted wall, with a garden staircase leading to a private upper
            floor. Guests come for one night between flights, or stay for weeks while working. Either way the
            space is yours: your own entrance, your own bathroom, your own terrace.
          </p>
          <p>
            Everything is handled quietly. Arrival instructions reach you before you land, the door opens on
            your schedule, and Markoni AI answers whatever comes up during your stay — with {info["host_name"]}{" "}
            and {info["cohost_name"]} a message away when a human is the right answer.
          </p>
          <Link to="/amenities" className="inline-block border-b border-foreground/30 pb-1 text-sm text-foreground">
          ...
          <div className="mt-12 grid gap-x-10 gap-y-12 sm:grid-cols-2 lg:grid-cols-4">
            {highlights.map((highlight, i) => (
              <Reveal key={highlight.title} delay={i * 60}>
                <h3 className="font-display text-2xl">{highlight.title}</h3>
      ...
      <section className="shell py-28">
        <Reveal className="flex flex-wrap items-end justify-between gap-6">
          ...
          <Link to="/rooms" className="border-b border-foreground/30 pb-1 text-sm">
            All rooms
          </Link>
        </Reveal>
      ...
      <section className="py-4">
        ...
        <div className="mt-8 grid grid-cols-2 gap-2 md:grid-cols-4">
          {gallery.slice(0, 8).map((image) => (
            <div key={image.url} className="img-zoom aspect-square bg-muted">
              <img src={image.url} alt={image.alt ?? ""} loading="lazy" className="size-full object-cover" />
            </div>
          ))}
        </div>
        <div className="shell mt-8">
          <Link to="/gallery" className="border-b border-foreground/30 pb-1 text-sm">
      ...
      <section className="shell grid gap-14 py-28 lg:grid-cols-12">
        <Reveal className="lg:col-span-4">
          ...
          <h2 className="mt-5 font-display text-4xl sm:text-5xl">Everything you need, already here.</h2>
          <Link to="/amenities" className="mt-8 inline-block border-b border-foreground/30 pb-1 text-sm">
            See the full list
          </Link>
        </Reveal>
      ...
      <section className="bg-ink py-28 text-ink-foreground">
        <div className="shell grid gap-14 lg:grid-cols-12">
          <Reveal className="lg:col-span-5">
            <p className="eyebrow text-ink-foreground/60">Getting here</p>
            <h2 className="mt-5 font-display text-4xl sm:text-5xl">
              Minutes from the runway, far from the noise.
            </h2>
            <p className="mt-6 text-ink-foreground/70">{info["airport_transport"]}</p>
            <Link
              to="/location"
          ...
          <div className="lg:col-span-6 lg:col-start-7">
            <ul className="divide-y divide-ink-foreground/12">
              {nearby.slice(0, 6).map((place, i) => (
                <Reveal as="li" key={place.name} delay={i * 50} className="flex justify-between gap-6 py-4">
                  <div>
                    <p className="text-ink-foreground">{place.name}</p>
                    <p className="text-xs text-ink-foreground/50">{place.category}</p>
                  </div>
      ...
      <section className="shell py-28">
        ...
        <div className="mt-14 grid gap-10 md:grid-cols-3">
          {reviews.map((review, i) => (
            <Reveal key={review.guest_name} delay={i * 90} className="flex flex-col border-t border-border pt-6">
              <StarRating rating={review.rating} />
              <p className="mt-5 flex-1 text-sm leading-relaxed text-muted-foreground">{review.body}</p>
              <p className="mt-6 text-sm">
                {review.guest_name}
                <span className="text-muted-foreground"> · {review.reviewed_on}</span>
              </p>
            </Reveal>
          ))}
        </div>
        <Reveal className="mt-12">
          <Link to="/reviews" className="border-b border-foreground/30 pb-1 text-sm">
            Read all reviews
          </Link>
        </Reveal>
      </section>
      ...
      <section className="bg-secondary py-28">
        <div className="shell grid gap-12 lg:grid-cols-2">
          <Reveal>
            ...
            <h2 className="mt-5 font-display text-4xl sm:text-5xl">
              An assistant that answers instantly, and knows when to hand over.
            </h2>
          </Reveal>
          <Reveal delay={120} className="space-y-5 text-muted-foreground">
            <p>
              From the moment you book, Markoni AI handles the practical things: arrival instructions, Wi-Fi
              details, late-night questions, directions to breakfast. Most guests never notice it isn't a person.
            </p>
            <p>
              When something needs judgement — a changed flight, an extended stay, a favour — it brings in your
              hosts straight away.
            </p>
          </Reveal>
        </div>
      ...
      <section className="shell py-28">
        ...
        <dl className="mt-10 divide-y divide-border border-y border-border">
          {faqs.slice(0, 6).map((faq, i) => (
            <Reveal key={faq.question} delay={i * 50} className="grid gap-3 py-7 md:grid-cols-12">
              <dt className="font-display text-xl md:col-span-5">{faq.question}</dt>
              <dd className="text-sm text-muted-foreground md:col-span-7">{faq.answer}</dd>
            </Reveal>
          ))}
        </dl>
        <Reveal className="mt-10">
          <Link to="/faq" className="border-b border-foreground/30 pb-1 text-sm">
            All questions
          </Link>
        </Reveal>
      </section>
      ...
      <section className="relative overflow-hidden">
        <img
          src={gallery[0]?.url ?? ""}
          alt={gallery[0]?.alt ?? "Zoe, the property"}
          loading="lazy"
          className="absolute inset-0 size-full object-cover"
        />
        ...
        <div className="shell relative py-32 text-center text-ink-foreground">
          ...
          <p className="mx-auto mt-5 max-w-lg text-ink-foreground/75">
            No fixed packages — one night or one month, the rate follows your stay.
          </p>
          <Link
            to="/book"
